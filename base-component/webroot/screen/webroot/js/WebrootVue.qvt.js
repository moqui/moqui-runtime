...
const { createApp,markRaw,defineComponent, h } = Vue;
const { loadModule } = window['vue3-sfc-loader'];

moqui.vueCompLoader = {};

moqui.vueCompLoader.fileMap = new Map();
moqui.vueCompLoader.options = {
  moduleCache: {
    vue: Vue,
  },
  getFile (url) {
    return moqui.vueCompLoader.fileMap.get(url)
  },
  addStyle (styleStr) {
...
webrootVueApp.config.globalProperties.moqui = moqui;
webrootVueApp.config.globalProperties.moment = moment;
webrootVueApp.config.globalProperties.window = window;
webrootVueApp.config.compilerOptions = {whitespace: 'preserve'};
    ...
    if (url.slice(-vueExt.length) === vueExt) {
        ...
        var vueAjaxSettings = { type:"GET", url:url, error:moqui.handleLoadError, success: function(resp, status, jqXHR) {
                ...
                if (moqui.isString(resp) && resp.length > 0) {
                    newUrl = url.replace(".qvue",'.vue')
                    moqui.vueCompLoader.fileMap.set(newUrl,resp);
                    loadModule(newUrl,moqui.vueCompLoader.options)
                        .then(newComponent =>{
                            //delete fileMap，saving memory
                            moqui.vueCompLoader.fileMap.delete(newUrl);

                            vueCompObj = markRaw(newComponent);
                        ...
                        .catch(err => {
                             console.error('Loading url %s error：%o',url, err);
                           });
    ...
    var ajaxSettings = { type:"GET", url:url, error:moqui.handleLoadError, success: function(resp, status, jqXHR) {
        ...
        if (moqui.isString(resp) && resp.length > 0) {
            if (isJsPath || resp.slice(0,7) === 'define(') {
                ...
                var jsCompObj = markRaw(eval(resp));
                ...
                var compObj = markRaw({ template: '<div' + (divId && divId.length > 0 ? ' id="' + divId + '"' : '') + '>' + templateText + '</div>' });
...
moqui.EmptyComponent = markRaw(defineComponent({
                                          name: 'EmptyComponent',
                                          render(){
                                            return h('div', { id: 'current-page-root' }, [h('div', { class: 'spinner' }, [h('div', '\u00A0')])])
                                            }
                                        }));
moqui.NotFound = markRaw(defineComponent({
                         name: 'NotFound',
                         render(){
                               return h('div', { id: 'current-page-root' }, h('h4', 'Screen not found at ' + this.$root.currentPath))
                         }

                       }));

/* ========== inline components ========== */
webrootVueApp.component('m-link', {
...
webrootVueApp.component('router-link', {
...
webrootVueApp.component('m-script', {
...
webrootVueApp.component('m-stylesheet', {
    name: "mStylesheet",
...
/* ========== layout components ========== */
webrootVueApp.component('m-container-box', {
    name: "mContainerBox",
...
webrootVueApp.component('m-box-body', {
    name: "mBoxBody",
    props: { height:String },
...
webrootVueApp.component('m-dialog', {
    name: "mDialog",
...
webrootVueApp.component('m-container-dialog', {
...
webrootVueApp.component('m-dynamic-container', {
...
webrootVueApp.component('m-dynamic-dialog', {
    name: "mDynamicDialog",
...
webrootVueApp.component('m-tree-top', {
    name: "mTreeTop",
...
webrootVueApp.component('m-tree-item', {
    name: "mTreeItem",
    template:
    '<li :id="model.id">' +
...
webrootVueApp.component('m-editable', {
    name: "mEditable",
...
webrootVueApp.component('m-checkbox-set', {
    name: "mCheckboxSet",
...
webrootVueApp.component('m-form', {
    name: "mForm",
    mixins:[moqui.checkboxSetMixin],
...
webrootVueApp.component('m-form-link', {
    name: "mFormLink",
...
webrootVueApp.component('m-form-paginate', {
    name: "mFormPaginate",
...
webrootVueApp.component('m-form-go-page', {
    name: "mFormGoPage",
...
webrootVueApp.component('m-form-column-config', {
...
webrootVueApp.component('m-form-list', {
    name: "mFormList",
...
webrootVueApp.component('m-date-time', {
    name: "mDateTime",
...
webrootVueApp.component('m-date-period', {
    name: "mDatePeriod",
...
webrootVueApp.component('m-display', {
    name: "mDisplay",
...
webrootVueApp.component('m-drop-down', {
    name: "mDropDown",
    ...
    template:
        ...
        '<q-select ref="qSelect" v-bind:model-value="value" @update:model-value="handleInput($event)"' +
                ' dense outlined options-dense use-input  :name="name" :id="id" :form="form"' +
...
webrootVueApp.component('m-text-line', {
    name: "mTextLine",
...
webrootVueApp.component('m-chart', {
    name: 'mChart',
...
webrootVueApp.component('m-mermaid', {
    name: 'mMermaid',
...
webrootVueApp.component('m-ck-editor', {
    name: 'mCkEditor',
...
webrootVueApp.component('m-simple-mde', {
    name: 'mSimpleMde',
    template:'<div><textarea ref="area"></textarea></div>',
...
webrootVueApp.component('m-subscreens-tabs', {
     ...
     */
    template:
    '<div v-if="subscreens.length > 1"><q-tabs dense no-caps align="left" active-color="primary" indicator-color="primary" v-model="activeTab">' +
...
webrootVueApp.component('m-subscreens-active', {
...
webrootVueApp.component('m-menu-nav-item', {
    name: "mMenuNavItem",
    props: { menuIndex:Number },
    template:
    '<q-expansion-item v-if="navMenuItem && navMenuItem.subscreens && navMenuItem.subscreens.length" default-opened :content-inset-level="0.3"' +
            ' switch-toggle-side dense dense-toggle expanded-icon="arrow_drop_down" :to="navMenuItem.pathWithParams" >' +
    ...
    '</q-expansion-item>' +
    '<q-expansion-item v-else-if="navMenuItem && navMenuItem.savedFinds && navMenuItem.savedFinds.length" default-opened :content-inset-level="0.3"' +
            ' switch-toggle-side dense dense-toggle expanded-icon="arrow_drop_down" :to="navMenuItem.pathWithParams" >' +
        '<template v-slot:header><m-menu-item-content :menu-item="navMenuItem" active></m-menu-item-content></template>' +
        '<template v-slot:default><q-expansion-item v-for="(savedFind, ssIndex) in navMenuItem.savedFinds" :key="savedFind.name"' +
                '  switch-toggle-side dense dense-toggle expand-icon="chevron_right" :to="savedFind.pathWithParams" >' +
            '<template v-slot:header><m-menu-item-content :menu-item="savedFind" :active="savedFind.active"/></template>' +
        '</q-expansion-item></template>' +
    '</q-expansion-item>' +
    '<q-expansion-item v-else-if="menuIndex < (navMenuLength - 1)" default-opened :content-inset-level="0.3"' +
            ' switch-toggle-side dense dense-toggle expanded-icon="arrow_drop_down" :to="navMenuItem.pathWithParams" >' +
        '<template v-slot:header><m-menu-item-content :menu-item="navMenuItem" active></m-menu-item-content></template>' +
        '<template v-slot:default><m-menu-nav-item :menu-index="menuIndex + 1"></m-menu-nav-item></template>' +
    '</q-expansion-item>' +
    '<q-expansion-item v-else-if="navMenuItem"  switch-toggle-side dense dense-toggle expand-icon="arrow_right" :to="navMenuItem.pathWithParams" >' +
        '<template v-slot:header><m-menu-item-content :menu-item="navMenuItem" active></m-menu-item-content></template>' +
    '</q-expansion-item>',
    methods: {
...
webrootVueApp.component('m-menu-subscreen-item', {
    ...
    '<q-expansion-item v-else  switch-toggle-side dense dense-toggle expand-icon="arrow_right" :to="subscreen.pathWithParams">' +
...
webrootVueApp.component('m-menu-item-content', {
...
webrootVueApp.use(router);

moqui.webrootVue = webrootVueApp.mount('#apps-root')
